use dup_check::{scanner::Scanner, cache::Cache};
...
fn test_full_duplicate_scan_workflow() {
    ...
    assert_eq!(duplicates.len(), 1, "Should still find one group of duplicates");
    for (_, files) in &duplicates {
...
fn test_scan_with_cache() {
    ...
    assert_eq!(duplicates1.len(), duplicates2.len(), "Cache should provide consistent results");
    
    // Verify that cache is being used
    let cache = Cache::new().unwrap();
    let file_path = temp_dir.path().join("file1.txt");
    assert!(cache.get_hash(&file_path, content.len() as u64).unwrap().is_some(),
...
fn test_scan_with_size_filters() {
    ...
    let duplicates = scanner.find_duplicates(temp_dir.path()).unwrap();
    assert_eq!(duplicates.len(), 2, "Should find medium and large duplicates");
    
    // Test maximum size filter
    let scanner = Scanner::new(false, None, Some(5000)).unwrap();
    let duplicates = scanner.find_duplicates(temp_dir.path()).unwrap();
    assert_eq!(duplicates.len(), 2, "Should find small and medium duplicates");
...
fn test_error_handling() {
    ...
    assert!(result.is_err(), "Should handle nonexistent directory gracefully");
    ...
    #[cfg(unix)]
    {
        ...
        assert!(result.is_err(), "Should handle unreadable directory gracefully");
    }
}
