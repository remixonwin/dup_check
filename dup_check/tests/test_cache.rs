...
fn test_cache_store_and_retrieve() {
    ...
    assert_eq!(
        retrieved,
        Some(hash),
        "Retrieved hash should match stored hash"
    );
}
...
fn test_cache_size_mismatch() {
    ...
    assert_eq!(
        retrieved, None,
        "Should not retrieve hash when size doesn't match"
    );
}
...
fn test_cache_multiple_files() {
    ...
    let files = vec![
        ...
        (
            create_temp_file(&temp_dir, "file2.txt", b"Different"),
            "hash2",
            9,
        ),
        (
            create_temp_file(&temp_dir, "file3.txt", b"Test data"),
            "hash3",
            9,
        ),
    ];
...
fn test_cache_concurrent_access() {
    ...
    for i in 0..10 {
        ...
        let handle = thread::spawn(move || {
            ...
            cache
                .store_hash(&file_path, content.len() as u64, &hash)
                .unwrap();
            ...
            assert_eq!(
                retrieved,
                Some(hash),
                "Retrieved hash should match in thread {}",
                i
            );
        });
